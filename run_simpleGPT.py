...
from prompt_template_simple import get_prompt_template
...
def load_model(device_type, model_id, model_basename=None, LOGGING=logging):
    ...
    if model_basename is not None:
        if ".gguf" in model_basename.lower():
            ...
            if llm is not None:
                logging.info('load_model function loaded a model')
            else:
                logging.info('load_model function failed to load a model. It is None')
            return llm
        elif ".ggml" in model_basename.lower():
            model, tokenizer = load_quantized_model_gguf_ggml(model_id, model_basename, device_type, LOGGING)
    else:
        logging.info('only .gguf models supported')
...
def retrieval_qa_pipline(device_type, use_history, promptTemplate_type="llama"):
    ...
    logging.info("retriver Loaded")

    # get the prompt template and memory if set by the user.
    prompt, memory = get_prompt_template(promptTemplate_type=promptTemplate_type, history=use_history)
    logging.info(f'loaded prompt: {prompt}')

    # load the llm pipeline
    llm = load_model(device_type, model_id=MODEL_ID, model_basename=MODEL_BASENAME, LOGGING=logging)
    if llm is not None:
        logging.info("llm is not None")
    else:
        logging.info("llm is None")
    if use_history:
